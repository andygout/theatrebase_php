...
<html>
<head>
  <title><?php echo $pagetab; ?> (setting: place - playtexts) | TheatreBase</title>
  ...
  <div id="container">
    ...
    <div id="content">
      <div id="results">
        ...
        if($k>0 && !empty($pt['plcs'])) { ?>
        <tr class="row<?php echo $rowclass; ?>">
          <td class="prdcol5" colspan="3">▪ <em><?php echo implode($pt['plcs'], ' / '); ?></em></td>
        </tr>
        <?php }
        if(!empty($pt['sg_pts'])) { ?>
        <tr class="row<?php echo $rowclass; ?>"><td class="ptcol4" colspan="3">
        <?php if($pt['sg_cnt']>count($pt['sg_pts'])) {echo 'Includes (with this place as setting):';} else {echo 'Comprises:';} ?></td></tr>
        ...
        <?php } else { ?></br><h5><em>NO PLAYTEXTS YET LISTED FOR THIS SETTING (PLACE)</em></h5><?php } ?>
      </div>
