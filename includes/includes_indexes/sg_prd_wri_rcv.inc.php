<?php
          $sql =  "SELECT coll_ov, wri_rl_id, src_mat_rl, wri_rl FROM prdwrirl
              INNER JOIN prd ON prdid=prd_id WHERE prdid='$sg_prd_id' AND coll_ov IS NOT NULL
              GROUP BY coll_ov, prd_id, wri_rl_id ORDER BY wri_rl_id ASC";
          ...
          $sql= "SELECT coll_ov, wri_rlid, mat_nm, frmt_nm FROM prdsrc_mat
              INNER JOIN mat ON src_matid=mat_id INNER JOIN frmt ON frmtid=frmt_id INNER JOIN prd ON prdid=prd_id WHERE prdid='$sg_prd_id' AND coll_ov IS NOT NULL
              GROUP BY coll_ov, prd_id, wri_rlid, mat_id ORDER BY src_mat_ordr ASC";
          $result=mysqli_query($link, $sql);
          ...
          $sql= "SELECT coll_ov, wri_rlid, wri_ordr, wri_sb_rl, comp_id, comp_nm FROM prdwri
              INNER JOIN comp ON wri_compid=comp_id INNER JOIN prd ON prdid=prd_id WHERE prdid='$sg_prd_id' AND wri_prsnid=0 AND coll_ov IS NOT NULL
              GROUP BY coll_ov, prd_id, wri_rlid, comp_id
              UNION
              SELECT coll_ov, wri_rlid, wri_ordr, wri_sb_rl, prsn_id, prsn_fll_nm FROM prdwri
              INNER JOIN prsn ON wri_prsnid=prsn_id INNER JOIN prd ON prdid=prd_id WHERE prdid='$sg_prd_id' AND wri_compid=0 AND coll_ov IS NOT NULL
          ...
          $sql= "SELECT coll_ov, wri_rlid, wri_compid, wri_sb_rl, prsn_fll_nm FROM prdwri
              INNER JOIN prsn ON wri_prsnid=prsn_id INNER JOIN prd ON prdid=prd_id WHERE prdid='$sg_prd_id' AND wri_compid!=0 AND coll_ov IS NOT NULL
              GROUP BY coll_ov, prd_id, wri_rlid, wri_compid, prsn_id ORDER BY wri_ordr ASC";
          $result=mysqli_query($link, $sql);
