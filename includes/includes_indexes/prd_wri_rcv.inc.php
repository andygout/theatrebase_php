<?php
            $sql="SELECT wri_rl_id, wri_rl, src_mat_rl FROM prdwrirl WHERE prdid='$prd_id' GROUP BY prdid, wri_rl_id ORDER BY wri_rl_id";
            ...
            $sql= "SELECT wri_rlid, mat_nm, mat_url, frmt_nm, frmt_url FROM prdsrc_mat
                  INNER JOIN mat ON src_matid=mat_id INNER JOIN frmt ON frmtid=frmt_id WHERE prdid='$prd_id'
            ...
            $sql= "SELECT wri_rlid, wri_ordr, wri_sb_rl, comp_id, comp_nm FROM prdwri
                  INNER JOIN comp ON wri_compid=comp_id WHERE prdid='$prd_id' AND wri_prsnid=0
                  GROUP BY prdid, wri_rlid, comp_id
                  UNION
                  SELECT wri_rlid, wri_ordr, wri_sb_rl, prsn_id, prsn_fll_nm FROM prdwri
                  INNER JOIN prsn ON wri_prsnid=prsn_id WHERE prdid='$prd_id' AND wri_compid=0
