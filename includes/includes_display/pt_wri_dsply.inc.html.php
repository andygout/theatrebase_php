        <tr class="row<?php echo $rowclass; ?>">
          ...
          $f=0; $rls=count($pt['wri_rls']); foreach($pt['wri_rls'] as $wri_rl):
          if(!empty($wri_rl['src_mats'])) {$g=0; $h=0; $sms=count($wri_rl['src_mats']); echo $wri_rl['src_mat_rl'].' ';
          foreach($wri_rl['src_mats'] as $src_mat):
          if(!preg_match("/^{$pt['pt_nm_pln']}$/i", $src_mat['src_mat_nm'])) {echo 'the '.$src_mat['src_mat_frmt'].', <span style="font-style:normal">'.$src_mat['src_mat_nm'].'</span>';
